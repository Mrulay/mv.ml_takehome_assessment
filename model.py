...
class Model:
    ...
    def __init__(
        self, trainfile="train.csv", testfile="test.csv", labelsfile="labels.csv"
    ):
    ...
    def preprocess(self, row):
        ...
        row = (
            lemmatizer.lemmatize(word) for word in row_tokens if word not in stopwords
        )  # Removing stopwords
    ...
    def get_vectors(self, row):
        ...
        for i, word in enumerate(row):
            if (
                word in self.w2v.wv.index_to_key
            ):  # If embedding for a word exists, replace it.
                row[i] = np.asarray(self.w2v.wv[word])
            else:
                row[i] = 0  # If the word is unknown, it will be a zero embedding
        row = (
            np.mean(row, axis=0)
        ).tolist()  # Averaging the vectors of all words in a document.
        return row
    ...
    def split(self, testsize):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            self.df.loc[:, self.df.columns != "label"],
            self.df.label,
            test_size=testsize,
            random_state=42,
        )
